...
%%% Setting the libsvm matlab porting Path
...
train_labels = train_labels(1:Ntrain); %%% get top Ntrain label, 1 dim
train_data   = train_data(1:Ntrain, :); %%% get top Ntrain data, 2 dims
test_labels  = test_labels(1:Ntest);
test_data    = test_data(1:Ntest,:);

%%% TODO setup SVM training options
%% TODO Set the option string for svm-train
% -t 2   :   Use the RBF Kernel
% -q     :   Quiet mode
% -c     :   Set scaling/penalty term
% -g     :   Set \gamma term
%% TODO RBF-Kernel Optimal Parameters
